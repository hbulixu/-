...
-(NSMutableArray *)travelassistantArray
{
    if (!_travelassistantArray) {
        ...
        NSString *endMoney = [self calculateAllConsumeWithDate:dict[@"endDate"]];
        //跟新数据库
...
-(void)setTravelCallBack:(NSString *)startPoint startDate:(NSString *)startDate endPoint:(NSString *)endPoint endFromDate:(NSString *)fromDate endToDate:(NSString *)toDate
{
    ...
    [self showTravelassistantView];
    //更改城市
    [self.travelassistantDb upDateTravelassistantWithCity:startPoint time:startDate cityType:@"出发点"];
    [self.travelassistantDb upDateTravelassistantWithCity:endPoint time:fromDate cityType:@"目的地"];
}

...
{
    if (!_noteArray) _noteArray = [self.db selectNoteAllDataWithCityType:self.noteType];
    return _noteArray;
}
...
- (void)viewDidLoad {
    ...
    self.db = db;
    if (![db selectTableWithTableName:noteTableName] || ![db selectTableDataWithTableName:noteTableName]) {
        //如果表不存在或者表的数据为空
...
{
    CGFloat margin = 20;
    UITableView *tableView = [[UITableView alloc] initWithFrame:CGRectMake(margin, 0, viewWidth - 2*margin, viewHeight)];
...
-(void)showCenterView
{
    //查询表看是否有记录,如果有记录则显示tableView,如果无则显示新增笔记
    if (![self.db selectTableDataWithTableName:noteTableName]) {
        if (self.setView) {
...
-(void)commentsCallBackWithText:(NSString *)text
{
    ...
    [self showCenterView];
    //更改行程助手表数据
    int noteCount = [self.db selectTravelassistantDataWithCityType:self.noteType colmnName:@"noteCount"];
    noteCount++;
    [self.db upDateTravelassistantWithColmnName:@"noteCount" colmnValue:noteCount cityType:self.noteType];
}
@end

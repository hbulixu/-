...
-(void)rightBarButtonItemClick
{
    ...
    if ([viewStr isEqualToString:@""]) {
        ...
        //创建和保存数据到数据库
        [self.travelassistantDb createTouristTable];
        [self.travelassistantDb insertTouristWithModel:model];
        
        //更改行程助手的数据
        int touristCount = [self.travelassistantDb selectTravelassistantDataWithCityType:self.cityType colmnName:@"touristCount"];
        touristCount++ ;
        [self.travelassistantDb upDateTravelassistantWithColmnName:@"touristCount" colmnValue:touristCount cityType:self.cityType];
...
-(void)addAccountClick
{
    ...
    if ([self.moneyItem.rightLabel.text intValue]) {
        ...
        //获取符号
        NSString *code = [self.db selectMoneySymbolWithCode:exrate];
        //金额
